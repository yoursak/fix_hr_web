...
class MonthlyAttendanceLivewire extends Component
{
    ...
    public function getDepartment()
    {
        if ($this->branchFilter != null) {
            return DepartmentList::join('employee_personal_details', 'department_list.depart_id', '=', 'employee_personal_details.department_id')
                ->where('employee_personal_details.branch_id', $this->branchFilter)
    ...
    {
        if ($this->branchFilter != null && $this->departmentFilter != null) {
            return DesignationList::join('employee_personal_details', 'designation_list.desig_id', '=', 'employee_personal_details.designation_id')
                ->where('employee_personal_details.branch_id', $this->branchFilter)
    ...
    public function render()
    {
        ...
        $query = EmployeePersonalDetail::where('employee_personal_details.business_id', $businessId)
        ...
        if ($permissionBranchId !== null && !empty($permissionBranchId) && $roleIdToCheck != 1 && $permissionBranchId->permission_type == 2) {
            ...
            $Emp = $query->where('employee_personal_details.branch_id', $permissionBranchId->permission_branch_id)
                ->paginate($page)
                ->withQueryString();
        } else {
            $page = $this->perPage != null ? $this->perPage : 10;
            $Emp = $query->paginate($page)
                ->withQueryString();
        }
        $designation = DesignationList::where('business_id', $businessId)->first();
        return view('livewire.attendance.monthly-attendance-livewire', compact('Emp', 'Branch', 'Department', 'Designation', 'totalDays', 'monthFilter'));
    }
}
