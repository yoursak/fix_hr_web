...
class AttendanceApiController extends Controller
{
    ...
    public function currentAttendanceStatus(Request $request)
    {
        ...
        $load = DB::table('attendance_list')->where('business_id', $business)->where('emp_id', $emp)->where('punch_date', $Today)->first(); //use by static data
    ...
    public function storeAttendance(Request $request) //remaining on faceID and location-tabs
    {
        ...
        $checkingModes = DB::table('attendance_mode')
        ...
        $information = DB::table('employee_personal_details')->where('business_id', $business)->where('emp_id', $emp)->first(); //findOut Employee Details

        // $method = $information->emp_attendance_method;
        ...
        $DATA = DB::table('employee_personal_details')
            ...
            ->whereRaw('JSON_CONTAINS(master_endgame_method.shift_settings_ids_list, JSON_QUOTE(employee_personal_details.emp_shift_type))') //End-Games method array index values checking
            ->where('master_endgame_method.method_switch', 1) //end game method on also
            ->where('attendance_shift_type_items.is_active', 1) //scheduler task shift is active check
            ...
            ->select(
                'master_endgame_method.id as method_id',
                'master_endgame_method.method_name as method_name',
                'attendance_shift_type_items.id as shift_item_id',
                'attendance_shift_type_items.shift_name as shift_template_name',
                'attendance_shift_type.id as shift_id as shift_type_id',
                'attendance_shift_type.name as shift_type_name',
                'attendance_shift_type_items.shift_start as shift_start_time',
                'attendance_shift_type_items.shift_end as shift_end_time',
                'attendance_shift_type_items.shift_hr as shift_hour',
                'attendance_shift_type_items.shift_min as shift_min',
                'attendance_shift_type_items.work_hr as working_hour',
                'attendance_shift_type_items.work_min as working_min',
                'attendance_shift_type_items.break_min as break_min',
                'attendance_shift_type_items.is_paid as is_paid'
            )
            ->first();
// dd($DATA);
        if (isset($DATA)) {
            ...
            if (
                ...
                if (isset($information)) {
                    ...
                    if ($emp != null && $business != null && $qrCode == 1 && $markedInOutMode != null) {
                        ...
                        $check = DB::table('attendance_list')->where('punch_date', $formattedDate)->where('emp_id', $emp)->where('emp_today_current_status', 1)->first();
                        if (isset($check)) {
                            if ($methodAccept != null && $punchOutTime != null && $punchOutAddress != null && $punchOutLongitude != null && $punchOutLatitude != null) {
                                ...
                                $loaded = DB::table('attendance_list')->where('punch_date', $formattedDate)->where('emp_id', $emp)->where('emp_today_current_status', 1)->update($updateDATA);
                            ...
                            if ($methodAccept != null && $punchInTime != null && $punchInTime != null && $punchInAddress != null && $punchInLatitude != null && $punchInLongitude != null) {
                                ...
                                $insertChecking = DB::table('attendance_list')->where('punch_date', $formattedDate)->where('emp_id', $emp)->where('emp_today_current_status', 2)->first();
    ...
    public function show($id)
    {
        ...
        if ($empData) {
            $attendData = DB::table('attendance_list')->where('emp_id', $id)->orderBy('id', 'desc')
                ->get();
