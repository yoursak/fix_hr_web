...
class RulesManagement
{
    ...
    static function ALLPolicyTemplates()
    {
        ...
        $leavePolicy = DB::table('setting_leave_policy')
            ->where('business_id', self::allValueGet()[5])
            ->get();
        $holidayPolicy = DB::table('holiday_template')
            ->where('business_id', self::allValueGet()[5])
            ->get();
        $weeklyPolicy = DB::table('weekly_holiday_list')
            ->where('business_id', self::allValueGet()[5])
            ->get();
        $attendanceMode = DB::table('attendance_mode')
            ->where('business_id', self::allValueGet()[5])
            ->get();
        $attendanceShiftSettings = DB::table('attendance_shift_settings')
            ->where('business_id', self::allValueGet()[5])
            ->get();
        $attendanceTrackPunchInOROut = DB::table('attendance_track_in_out')
            ->where('business_id', self::allValueGet()[5])
        ...
        $finalEndGameRule = DB::table('master_endgame_method')
            ...
            ->where('master_endgame_method.business_id', self::allValueGet()[5])
            ->select('master_endgame_method.*', 'attendance_endgame_policypreference.policy_name as policy_name', 'attendance_endgame_level.level_name as level_name')
            ->get();
        ...
        $employeeInfomation = DB::table('employee_personal_details')
            ...
            ->join('attendance_shift_type', 'employee_personal_details.emp_shift_type', '=', 'attendance_shift_type.id')
        ...
        $adminRoleList = DB::table('setting_role_create')
            ->where('business_id', self::allValueGet()[5])
            ->get();
        // Rule List
        // $lateentry;
        ...
        //     ->join('attendance_endgame_policypreference', 'master_endgame_method.policy_preference', '=', 'attendance_endgame_policypreference.id')
        //     ->join('attendance_endgame_level', 'master_endgame_method.policy_preference', '=', 'attendance_endgame_level.policypreference_level_id')
        //     ->join('attendance_shift_settings', function ($join) use ($attendanceShiftSettings) {
        //         $join->on('master_endgame_method.shift_settings_ids_list', 'LIKE', DB::raw("CONCAT('%', attendance_shift_settings.id, '%')"))
        //             ->whereIn('attendance_shift_settings.id', $attendanceShiftSettings);
        //     })
        //     ->where('master_endgame_method.business_id', self::allValueGet()[5])->get();
        // dd($finalEndGameRule);
        // ->join('attendance_shift_settings', 'master_endgame_method.shift_settings_ids_list', '=', 'attendance_shift_settings.id')
    ...
    static function ALLPolicyTemplatesByIDCall($BID)
    {
        ...
        $attendanceMode = DB::table('attendance_mode')
            ...
            ->get();
        $attendanceShiftSettings = DB::table('attendance_shift_settings')
            ->where('attendance_shift_settings.business_id', $BID)
        ...
        $finalEndGameRule = DB::table('master_endgame_method')
            ->join('attendance_endgame_policypreference', 'master_endgame_method.policy_preference', '=', 'attendance_endgame_policypreference.id')
            ->join('attendance_endgame_level', 'master_endgame_method.policy_preference', '=', 'attendance_endgame_level.policypreference_level_id')
            ->where('master_endgame_method.business_id', $BID)
            ->select('master_endgame_method.*', 'attendance_endgame_policypreference.policy_name as policy_name', 'attendance_endgame_level.level_name as level_name')
            ->get();
    ...
    static function GetValues($id)
    {
        ...
        $leaveSettingsList = DB::table('setting_leave_policy')
            ->where('id', $id)
            ->where('business_id', self::allValueGet()[5])
            ->first();
        $holidayPolicyList = DB::table('holiday_template')
            ->where('temp_id', $id)
            ->where('business_id', self::allValueGet()[5])
            ->first();
        $weeklyPolicyList = DB::table('weekly_holiday_list')
            ->where('id', $id)
            ->where('business_id', self::allValueGet()[5])
            ->first();

        // attendance
        $AttendanceShiftPolicyList = DB::table('attendance_shift_settings')
            ->where('attendance_shift_settings.id', $id) // Prefix with table name
            ->join('attendance_shift_type', 'attendance_shift_settings.shift_type', '=', 'attendance_shift_type.id')
            ->where('attendance_shift_settings.business_id', self::allValueGet()[5])
            ->first();
        $attendanceModeList = DB::table('attendance_mode')
            ->where('business_id', self::allValueGet()[5])
            ->first();
    ...
    {
        $load = DB::table('attendance_methods')->get();
        $attendanceData = [];
    ...
    {
        $load_Attendance_Mode = DB::table('attendance_mode')
            ->where('business_id', self::$BusinessID)
            ->first();
    ...
    {
        $load_Attendance_ShiftCount = DB::table('attendance_shift_settings')
            ->where('business_id', self::$BusinessID)
            ->count();
    ...
    {
        $checked = DB::table('attendance_shift_settings')
            ->where('id', $ID)
            ->where('business_id', self::$BusinessID)
            ->first();
        if ($checked != null) {
    ...
    {
        return [self::allValueGet()[4], self::allValueGet()[5]];
    }
