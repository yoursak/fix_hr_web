...
class Central_unit
{
   ...
   public static function RoleIdToName2($roleId)
   {
      ...
      $result = DB::table('setting_role_create')->where('id', $roleId)->select('roles_name')->first();
   ...
   {
      $load = DB::table('designation_list')->where('desig_id', $ID)->select('desig_name')->first();
      if (isset($load)) {
   ...
   {
      return DB::table('department_list')->where(['depart_id' => $id, 'b_id' => self::$BusinessID])->select('depart_name')->first();
   }
   static function Branchget($id)
   {
      return DB::table('branch_list')->where(['branch_id' => $id, 'business_id' => self::$BusinessID])->select('branch_name')->first();
   }
   ...
   {
      return DB::table('setting_role_items')->where('role_create_id', $roleId)->select('model_name as roleName')->first();
   }
   ...
   {
      $Branch = DB::table('branch_list')::where('branch_id', $branch_id)->first();
   }
}