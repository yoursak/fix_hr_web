...
<div class="row col-md-12">
    ...
    <div class="col-md-4 p-2">
        ...
        <x-livewire-powergrid::inline-filters :checkbox="$checkbox" :actions="$actions" :columns="$columns"
            :theme="$theme" :filters="$filters" :enabledFilters="$enabledFilters" :tableName="$tableName"
            :setUp="$setUp" />
    </div>
...
<x-livewire-powergrid::table-base :theme="$theme->table" :ready-to-load="$readyToLoad">
    <x-slot:header>
        <tr class="{{ $theme->table->trClass }}" style="{{ $theme->table->trStyle }}">
            ...
            <x-livewire-powergrid::cols :column="$column" :theme="$theme" :enabledFilters="$enabledFilters" />
            @endforeach
            ...
            $isActionFixedOnResponsive = isset($this->setUp['responsive']) && in_array($responsiveActionsColumnName,
            data_get($this->setUp, 'responsive.fixedColumns')) ? true : false;
            @endphp

            <th @if($isActionFixedOnResponsive) fixed @endif
                class="{{ $theme->table->thClass . ' ' . $theme->table->thActionClass }}" scope="col"
