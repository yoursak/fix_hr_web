...
const headerText = Array.from(visibleHeaders)
  .map(header => header.innerText.trim())
  .join('\t'); // Use '\t' to separate headers, you can adjust this as needed

// Copy header text to clipboard
navigator.clipboard.writeText(headerText)
  .then(() => {
    console.log("Headers copied to clipboard:", headerText);
    // Perform any other actions or feedback upon successful copy
  })
  .catch(err => {
