...
<style>
    .custom-tooltip .tooltip-inner {
        color: #000; /* Set your desired text color for the tooltip title */
    }
                        ...
                        {{-- <table id="file-datatable"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-bottom-0">cycle Type</th>
                            ...
                            <tbody class="my_body">
                                ...
                                if($checkApprovalCycleType==1)
                                {
                                    ...
                                    ->where('approval_type_id',2)->where('applied_cycle_type',1)->where('emp_id',$loginEmpID)->where('role_id',$loginRoleID)->where('all_request_id',$item->id)->first();
                                    echo '1';
                                }if($checkApprovalCycleType==2)
                                {
                                    $current_status_particular_tb = DB::table('approval_status_list')
                                    ->where('approval_status_list.approval_type_id',2)->where('approval_status_list.applied_cycle_type',2)->where('approval_status_list.all_request_id',$item->id)->first();
                                    echo '2';
                                }
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php 
                                        // $ad=$RuleManagement::FinalRequestStatusSubmitFilterValue($item->id,2)[0];
                                        // print_r($ad);
                                            $checkingCover=  DB::table('approval_status_list')->where('business_id', $loginRoleBID)->where('all_request_id',$item->id)->first();
                                        if($checkApprovalCycleType==1){
                                              // finds Pending Status
                                                if($checkingCover!=null)
                                                {
                                                    $checkingCoverss=  DB::table('approval_status_list')->where('business_id', $loginRoleBID)->where('role_id',$loginRoleID)->where('all_request_id',$item->id)->first();
                                                 
                                                    if($checkingCoverss){
                                                    // echo 'Pending Nahi hy'.$checkingCover->id.' ';
                                                    ...
                                                    ->first();
                                            // ->where('approval_status_list.status', '!=', 0)
                                            ...
                                            if($CheckCurrentStaticStatus!=null){
                                                ...
                                                if($item->process_complete==1)
                                                {
                                                    ...
                                                    ->where('request_leave_list.id',$item->id)->first();
                                                    ?>
                                                    <small>
                                                    Finished To <?=$SD->final_status?>
                                                    </small>
                                                    
                                    <span class="<?=$SD->request_color ?>">
                                            <?= $SD->request_response ?>
                                              ...
                                              <small class="<?=$CheckCurrentStaticStatus->request_color?>"   data-bs-trigger="hover"     data-bs-container="body" data-bs-content="Forward To <?=$ForwardName?>" data-bs-placement="top" data-bs-popover-color="primary" data-bs-toggle="popover" title="<?=$CheckCurrentStaticStatus->request_response?>" data-bs-original-title="Forward To <?=$ForwardName?>">
                                                <?=$CheckCurrentStaticStatus->request_response?>
                                              </small>
                                                   ...
                                                   $checkingLoad= DB::table('approval_management_cycle')
                                                    ...
                                                    ->select('role_id')
                                                    ->first();
                                                    $roleIds = json_decode($checkingLoad->role_id, true); // Decode the JSON array
                                                    $currentIndex = array_search($loginRoleID, $roleIds); // Find the current index of forward_by_role_id
                                                    $nextIndex = $currentIndex + 1;
                                                    $nextRoleId = isset($roleIds[$nextIndex]) ? $roleIds[$nextIndex] : -1; //sensitive case if last next end then recall 0
                    
                                                    $ApprovedName=DB::table('approval_status_list')->where('all_request_id',$item->id)->where('role_id',$nextRoleId)->first();
                                                   
                                                    // $approved=$ApprovedName->forward_by_role_id;
                                                    // // finding approval layer
                                                    // $checkingApprovalChange=DB::table('approval_status_list')->where('next_role_id',$loginRoleID)->first();
                                                    // dd($ApprovedName);
                                                    // if($ApprovedName!=null)
                                                    // {  
                                                        $gotopow= $RuleManagement::RoleName($ApprovedName->role_id??0)[0];
                                                        $ApprovedName2=$gotopow->roles_name??0;
                                                     
                                                        ?>

                                                   <?php echo '<br> Approved To '.$ApprovedName2; ?>
                                                   <?php //}
                                             ...
                                             ?>
                                        <!-- <span class="<?=$CheckCurrentStaticStatus->request_color ?>">
                                            <?= $CheckCurrentStaticStatus->request_response ?>
                                        </span> -->
                                        ...
                                        <?php
                                            ...
                                            ->join('static_status_request', 'approval_status_list.status', '=', 'static_status_request.id')
                                            ->where('approval_status_list.all_request_id',$item->id)->where('approval_status_list.next_role_id',$loginRoleID)
                                            ->first();
                                            if($requestGet??false)
                                            { 
                                            ...
                                            $ApprovedName=$CheckingRole->roles_name??0;
                                                ?>
                                        <small><?=$requestGet->request_response?> By <?=$ApprovedName?>
                                        </small>
                                        <span class="badge badge-warning-light">Padding
                                        </span> 
                                       ...
                                       <?php     }
                                        ...
                                        <?php
                                            ...
                                            ->join('static_status_request', 'approval_status_list.status', '=', 'static_status_request.id')
                                            ->where('business_id', $loginRoleBID)
                                            // ->where('approval_status_list.applied_cycle_type', $checkApprovalCycleType)
                                            // ->where('approval_status_list.status', '!=', 0)
                                            ->where('approval_status_list.all_request_id', $item->id)
                                            ->first();
                                          if (!empty($CheckCurrentStaticStatus)) { 
                                            ?>
                                        <span class="<?=$CheckCurrentStaticStatus->request_color ?>">
                                            <?= $CheckCurrentStaticStatus->request_response ?>
                                        </span>
                                        ...
                                        <?php
                                                    ...
                                                    ?>
                                        {{-- @if ($item->forward_by_status == 0)
                                        <span class="badge badge-primary-light">Requested</span>
                                        @endif
                                        @if ($item->forward_by_status == 1)
                                        <span class="badge badge-success-light">Approved</span>
                                        @endif
                                        @if ($item->forward_by_status == 2)
                                        <span class="badge badge-warning-light">Declined</span>
                                        @endif
                                        @if ($item->forward_by_status == 3)
                                        <span class="badge badge-warning-light">Padding</span>
                                        @endif --}}

                                    </td>
                                    <td>
                                        <?=$checkApprovalCycleType??0?>
                                    </td>
                                    <td>
                                        {{-- data-status='
                                        <?= //$item->status ?>' --}}
                                        ...
                                        <a class="btn btn-primary btn-icon btn-sm " href="javascript:void(0);"
                                            id="edit_btn_modal" onclick="openEditModel(this)" data-id='<?= $item->id ?>'
            ...
            <form action="{{ route('admin.leaveapprove') }}" method="post">
                ...
                <div class="modal-body px-5 ">
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" class="form-control" id="editDepratment" placeholder="" value=""
                                readonly>
                        </div>
                        <div class="form-group col-md-4">
                            <label for="inputPassword4">Designation</label>
                            <input type="text" class="form-control" id="editDesignation" placeholder="" value=""
                                readonly>
                        </div>
                        <div class="form-group col-md-4">
                        ...
                        <div class="form-group  col-md-4">
                            ...
                            <input type="text" class="form-control" id="editMobileNo" placeholder="Mobile No." value=""
                                readonly>
                        </div>
                    </div>
                    ...
                    <div class="form-row" id="leaveTypeOneShow">
                        <div class="form-group  col-lg-2 col-md-4">
                            ...
                            <select name="time_type" class="form-control" onchange="checkLeaveType(this)" value=""
                                id="editLeaveTypeFirst">
                        ...
                        <div class="form-group  col-lg-2 col-md-4">
                            ...
                            <select name="time_type" class="form-control" value="" id="editLeaveCategoryFirst">
                        ...
                        <div class="form-group  col-lg-3 col-md-4">
                            <label for="inputPassword4">From <i class="fa fa-calendar" data-bs-toggle="tooltip" title=""
                                    data-bs-original-title="fa fa-calendar" aria-label="fa fa-calendar"></i></label>
                            <input type="date" class="form-control" id="editFrom" name="from_date" placeholder="time"
                                value="">
                        </div>

                        <div class="form-group  col-lg-3 col-md-4">
                            <label for="inputPassword4">To <i class="fa fa-calendar" data-bs-toggle="tooltip" title=""
                                    data-bs-original-title="fa fa-calendar" aria-label="fa fa-calendar"></i></label>
                            <input type="date" class="form-control" name="to_date" id="editTo" placeholder="Password"
                                value="">
                        </div>
                    ...
                    <div class="form-row" id="leaveTypeTwoShow">
                        <div class="form-group  col-lg-2 col-md-4">
                            <label for="inputPassword4">Leave Type</label>
                            <select name="time_type" class="form-control" value="" onchange="checkLeaveType(this)"
                                id="editLeaveTypeSecond">
                                <option value="">Select Type</option>
                                @foreach ($leaveType as $leavetypes)
                                <option value="{{ $leavetypes->id }}">
                                    {{ $leavetypes->leave_day }}
                        ...
                        <div class="form-group col-lg-2 col-md-4">
                            <label for="inputPassword4">Leave Category</label>
                            <select name="time_type" class="form-control" value="" id="editLeaveCategorySecond">
                                <option value="">Select Category</option>
                                @foreach ($leaveCategory as $leavcat)
                                <option value="{{ $leavcat->id }}">
                                    {{ $leavcat->category_name }}
                        ...
                        <div class="form-group col-lg-2 col-md-4">
                            <label for="inputPassword4">Days</label>
                            <input type="text" class="form-control" id="editDaysSecond" name="days1" placeholder="days">
                        </div>
                    </div>
                    <div class="form-row">
                    ...
                    <div class="form-row d-none" id="remarks">
                        <div class="form-group col">
                            ...
                            <textarea class="form-control required" id="RemarkTextarea" name="remark" rows="2"
                                value=""></textarea>
                        </div>
                    </div>
                ...
                <div class="modal-footer" id="editModalFooter">
                    ...
                    <div class="d-flex m-0 ">
                        <span class="btn btn-danger mx-2" id="CancelBtn_MGA" data-bs-dismiss="" type="cancel " name=""
                            onclick="remark()" value="">Decline</span>
