        ...
        <div class="card">
            @empty(!$Employee)
                ...
                @foreach ($Employee->where('role_id', '!=', null)->where('role_id', '!=', 0) as $admin)
                    <div class="card-body border-bottum-0">
                        <div class="row">
                            <div class="col-6 col-xl-3 my-auto d-flex">
                                <span class="avatar avatar-md brround me-3"
                                    style="background-image: url('/storage/livewire_employee_profile/{{ $admin->profile_photo }}')"></span>
                                <h5 class="my-auto">
                                    <a href="{{ route('employeeProfile', [$admin->emp_id]) }}">
                                    {{ ($admin->emp_name ?? '') . ' ' . ($admin->emp_mname ?? '') . ' ' . ($admin->emp_lname ?? '') }}<br>
                                    <p class="text-muted mb-0 fs-12">
                                        <?= $rooted->DesingationIdToName($admin->designation_id) ?>
                                    </p>
                                </a>
                                </h5>
                                ...
                                <p class="my-auto text-muted text-end">
                                    ...
                                    @if (in_array('Admin List.Delete', $permission) || in_array('Admin List.All', $permission) )
                                        <a class="btn btn-danger btn-icon btn-sm" href="javascript:void(0);"
                                            onclick="ItemDeleteModel(this)" data-id='<?= $admin->emp_id ?>' data-role_id='<?= $Rolee->id ?>'
